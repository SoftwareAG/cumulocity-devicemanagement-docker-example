...
import streamingAnalytics.modelExchange
import sys
import os
...
def operationClassification(operation, operationID):
    try:
        if 'c8y_Restart' in str(operation):
            ...
            try:
                API.operations.setOperationMode(operationID, 'EXECUTING')
                newModel = streamingAnalytics.modelExchange.NewModel(operation['c8y_ThinEdge_Model']['name'], operation['c8y_ThinEdge_Model']['type'], operation['c8y_ThinEdge_Model']['id'],operation['c8y_ThinEdge_Model']['order'])
                newModel.main()
                API.operations.setOperationMode(operationID, 'SUCCESSFUL')
            except Exception as e:
                logger.error('The following error occured: %s'%(str(e)))
